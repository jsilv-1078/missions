...
async function ch<T>(path:string,body:unknown):Promise<T>{const key=process.env.CARDHEDGE_API_KEY;if(!key)throw new Error('Market data key is not configured');const r=await fetch(API_BASE+path,{method:'POST',headers:{'X-API-Key':key,'Content-Type':'application/json'},body:JSON.stringify(body),cache:'no-store',signal:AbortSignal.timeout(20000)});if(!r.ok)throw new Error(`Market data request returned ${r.status}`);return r.json() as Promise<T>}
...
async function detailFor(card:Card,costBasis:number){
 const selected=normCard(card)
 const grade=card.prices?.find(x=>x.grade==='PSA 10')?'PSA 10':card.prices?.[0]?.grade||'PSA 10'
 const [fmvRes,compsRes]=await Promise.allSettled([
  ch<any>('/v1/cards/card-fmv',{card_id:card.card_id,grade}),
  ch<any>('/v1/cards/comps',{card_id:card.card_id,count:10,grade,include_raw_prices:true,time_weighted:true})
 ])
 const fmvPayload=fmvRes.status==='fulfilled'?fmvRes.value:{}
 const fmv=fmvPayload?.fmv??fmvPayload
 const current=Number(fmv?.price??selected.value)
 const comps=compsRes.status==='fulfilled'?compsRows(compsRes.value):[]
 const compPrices=comps.map((x:any)=>x.price)
 const gain=current-costBasis
 return {...selected,value:current,displayValue:money(current),paid:costBasis,gain,displayGain:`${gain>=0?'+':'−'}$${Math.abs(gain).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}`,confidence:String(fmv?.confidence_grade??''),freshnessDays:Number(fmv?.freshness_days??0),rangeLow:compPrices.length?Math.min(...compPrices):0,rangeHigh:compPrices.length?Math.max(...compPrices):0,comps}
}
...
export async function GET(){
 try{
  ...
  const holdings=collectionRaw.map((c,i)=>{const n=normCard(c);const paid=costBases[i]??0;const gain=n.value-paid;return{...n,paid,gain,displayGain:`${gain>=0?'+':'−'}$${Math.abs(gain).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}`}})
  const details=await Promise.all(collectionRaw.map((card,i)=>detailFor(card,costBases[i]??0)))
  return NextResponse.json({live:true,updatedAt:new Date().toISOString(),movers,holdings,details,detail:details[0]??null},{headers:{'Cache-Control':'no-store'}})
 }catch(error){return NextResponse.json({live:false,error:error instanceof Error?error.message:'Unknown market data error',updatedAt:new Date().toISOString()},{status:502,headers:{'Cache-Control':'no-store'}})}
}
