...
test("followed players move earlier without breaking player spacing",() => {
  const stories = Array.from({ length:18 },(_,index) =>
    market(`card-${index}`,index === 5 || index === 12 ? "Kobe Bryant" : `Player ${index}`,"Basketball","rookie_watch"),
  );

  for (let run = 0; run < 20; run += 1) {
    const ordered = remixFeedStories(stories,undefined,{ followedPlayers:["Kobe Bryant"] });
    assert.equal(ordered[0].player,"Kobe Bryant");
    const kobeIndexes = ordered.flatMap((story,index) => story.player === "Kobe Bryant" ? [index] : []);
    assert.ok(kobeIndexes[1] - kobeIndexes[0] > 6,"followed player stories still need breathing room");
  }
});

test("show-less players move later but remain available",() => {
  const stories = Array.from({ length:10 },(_,index) =>
    market(`card-${index}`,index === 0 ? "Michael Jordan" : `Player ${index}`,"Basketball","grade_gap"),
  );

  for (let run = 0; run < 20; run += 1) {
    const ordered = remixFeedStories(stories,undefined,{ lessPlayers:["Michael Jordan"] });
    assert.equal(ordered.at(-1)?.player,"Michael Jordan");
    assert.equal(ordered.length,stories.length);
  }
});
